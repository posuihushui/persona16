  ...
  return (
    <div className="type-grid-wrap">
      <div className="type-grid">
        ...
        {rowKeys.flatMap((row) => [
          ...
          ...colKeys.map((col) => {
            ...
            return (
              <Link key={code} href={`/t/${pack.meta.slug}/type/${code}`} className="type-cell">
                <TypeArt code={code} size={44} className="cell-art" />
                <b>{code}</b>
                <span>{doc.name}</span>
              </Link>
            );
      ...
      <p className="small muted" style={{ margin: "0.75rem 0 0" }}>
        每一列的两个字母来自「{dimOf(order[1])?.name ?? ""}」和「{dimOf(order[2])?.name ?? ""}」，
        每一行来自「{dimOf(order[0])?.name ?? ""}」和「{dimOf(order[3])?.name ?? ""}」。
      </p>
    </div>
  );
}
